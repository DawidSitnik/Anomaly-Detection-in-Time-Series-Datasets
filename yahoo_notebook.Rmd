...
Calculate F1 score
```{r}
print_f_score <- function(truth, prediction) {
    labels = data.frame("truth" = as.factor(truth), 
                      "prediction" = as.factor(prediction))
    labels %>% f_meas(truth, prediction) %>% print()
}

print_f_score(series$is_anomaly, anomalies_STAT(series))
```
...
F1 score
```{r}
print_f_score(series$is_anomaly, series_ocsvm$is_anomaly)
```
...
Precisions
```{r}
print_precision(series$is_anomaly, anoms_esd$is_anomaly)
```

Recall
```{r}
print_recall(series$is_anomaly, anoms_esd$is_anomaly)
```

F1 score
```{r}
print_f_score(series$is_anomaly, anoms_esd$is_anomaly)
```
...
F1 score
```{r}
print_f_score(series$is_anomaly, series_iso$is_anomaly)
```
