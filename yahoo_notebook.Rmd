...
display_anomalies <- function(series, detection_func) {
  series_stat <- series
  series_stat$is_anomaly <- 0
  series_stat["is_anomaly"] <- detection_func(series)
  plot_ts(series_stat)
}

display_anomalies(series, anomalies_STAT)
```
...
plot_conf_mat(series$is_anomaly, series_stat$is_anomaly)
```
...
print_recall(series$is_anomaly, series_stat$is_anomaly)
```
