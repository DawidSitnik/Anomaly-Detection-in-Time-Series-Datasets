...
We can identify anomalies based on the 5% percentile values.
```{r, warning=FALSE}
...
plot_ts(series_stat, "Statistical anomalies")
```
...
```{r}
series_embeddings <- embed(series$value, 4)
...
plot_ts(series, "True anomalies")
plot_ts(series_ocsvm, "SVM anomalies")
```
...
```{r, cache=TRUE}
plot_ESD_confusion <- function(series, predicted) {
